...
**There are four readers of that condition now** (L7), and all four live in `domain/billable.ts` beside it: the billable list, `billingStateOf` for one row, `unbilledCentsOf` for the money in a summary line, and `activityBillingCondition` for the list filter and the counts behind the "Abgerechnet" / "Nicht abgerechnet" chips. The last one is a predicate over an `activity` row rather than a count for one id, which is the only reason it is a second function at all — and it is written where it is so that it cannot become a second *definition*. A filter that decided this on its own would agree with the badge on every row until an invoice was cancelled, and then a list would put a row under "Abgerechnet" that badges itself "Nicht abgerechnet".
