...
**What the list does not filter by is the appointment.** There is no chip for a `appointment.status` and none for "without a Termin": the tab is about Vorgänge. The slot's status still stands in every row as a badge, because a cancelled appointment is worth seeing while skimming — being worth *seeing* and being worth *filtering by* are different questions, and answering the second one automatically is how a row of six chips becomes eleven.
